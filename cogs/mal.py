...
class Mal(commands.Cog):
    ...
    async def mal(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await ctx.send('Invalid Syntax. See `>help mal` for more info on how to use this command.')
    ...
    async def anime(self, ctx, *, msg: str = None):
        ...
        if msg:
            fetch = await ctx.send('Searching...')
            ...
            if not embed_perms(ctx.message) or link is True:
                await ctx.send('https://myanimelist.net/anime/%s' % results.id)
                await fetch.delete()
                return await ctx.message.delete()
            ...
            await ctx.send('Specify an anime to search for.')
    ...
    async def manga(self, ctx, *, msg: str = None):
        ...
        if msg:
            ...
            if not embed_perms(ctx.message) or link is True:
                await ctx.send('https://myanimelist.net/manga/%s' % results.id)
                await fetch.delete()
                return await ctx.message.delete()
    ...
    async def next_(self, ctx, *, query):
        ...
        if found:
            ...
            try:
                ...
                await ctx.send(":exclamation: Oops!\n {}: {}".format(type(e).__name__, e))
                await search.delete()
                return await ctx.message.delete()
        else:
            await ctx.send('Failed to find given anime.')
            await search.delete()
            return await ctx.message.delete()
