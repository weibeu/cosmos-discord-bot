...
async def dump_nick(guild_id, user_id, nick):
    ...
    if await count_points(guild) == 0:
        await insert(guild, {'_id': 'members'})
    await guild.update_one({'_id': 'members'}, {'$addToSet': {str(user_id)+".nicks": nick}})
...
async def dump_username(guild_id, user_id, username):
    ...
    if await count_points(guild) == 0:
        await insert(guild, {'_id': 'members'})
    await guild.update_one({'_id': 'members'}, {'$addToSet': {str(user_id)+".usernames": username}})
...
async def create_tag(guild_id, user_id, tag_name, content):
    ...
    if await count_points(guild)==0:
        await insert(guild, {'_id': 'members'})
    await guild.update_one({'_id': 'members'}, {'$set': {str(user_id)+".tags."+tag_name.lower(): content}})
...
async def create_tag_box(guild_id, tag_name, content):
    ...
    await guild.update_one({'_id': 'tag-box'}, {'$set': {tag_name.lower(): content}})
...
async def unset_member_role(guild_id, member_id, role_id):
    guild = motor_client.guilds[str(guild_id)]
    await guild.update_one({'_id': 'members'}, {'$unset': {str(member_id)+".roles-rs."+str(role_id): ""}})
...
async def get_spook_data(guilds):
    ...
    for g in guilds:
        guild = motor_client.guilds[str(g.id)]
        s = await guild.find_one({'_id': 'settings'}, {'not_spook': '$'})
        if s is None:
            pass
        else:
            try:
